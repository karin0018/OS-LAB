...
#define PACK_PREV_ALLOC(val, prev_alloc) ((val) | (prev_alloc << 1))
...
static void *coalesce(void *bp)
{
    ...
    else /*00 next_block & prev_block are all free*/
    {
        size_t prev_prev_alloc = 0;
        delete_from_free_list(NEXT_BLKP(bp));
        delete_from_free_list(PREV_BLKP(bp));
...
static void place(void *bp, size_t asize)
{
    ...
    {
        PUT(HDRP(bp),PACK(asize,1,1)); /*?????????????????GET_PREV_ALLOC(HDRP(bp)) == 1 ?? 0 ??????*/
        bp = NEXT_BLKP(bp);
    ...
    else
    {
        ...
        bp = NEXT_BLKP(bp);
        PUT(HDRP(bp),PACK(GET_SIZE(HDRP(bp)),1,GET_ALLOC(HDRP(bp)))); /*?????????????*/
    }
}
...
static void add_to_free_list(void *bp)
{
    ...
    {
        SET_PRED(bp, 0);
        SET_SUCC(bp, (size_t)free_listp); /*typedef unsigned long long size_t */
